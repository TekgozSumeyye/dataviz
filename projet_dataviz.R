...
ggplot(fulldt) +
  ...
  theme_minimal()
#En semaine, la consommation d'alcool est plutot modÈrer (tres peu de consommation)
...
ggplot(fulldt) +
  ...
  geom_histogram(bins = 30L, fill = "#112446") +
  theme_minimal()
#Il y a beaucoup plus de consommation le weekend, Ètant donnÈ qu'il n'y a pas cours, plus de personnes consomment excessifement de l'alcool. 
...
#Cela peut etre du au fait qu'en semaine, les personnes consommant de l'alcool ne se concentre pas aux revisions donc leurs notes sont impactÈs. 
...
#Les notes mÈdians sont plus eleves pour les filles avec environ 12 et 11 pour les garcons. 
...
ggplot(fulldt) +
  ...
  theme_bw()
#Pour les boites a moustache du metier de la mere, il y a 6 valeurs aberrantes pour la categorie "autres", ce sont des valeurs qui sont supÈrieures ou infÈrieures aux limites dÈfinies par les moustaches. 
#On remarque que les eleves de mere travaillant dans la sante ou les eleves de meres professeurs ont des meilleurs notes (mÈdiant) que les meres aux foyers et "autres". 
...
#b)Recherche plus poussee des raisons d'une consommation d'alcool ´excessiveª :
#...
